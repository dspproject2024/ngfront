...
export class StatusService {
  private apiUrl = 'https://localhost:8000/api/statuses'; // Replace with your actual API URL

  constructor(private http: HttpClient) {}

  // Helper method to get the headers with the authorization token and correct Content-Type
  private getAuthHeaders(contentType: string = 'application/ld+json'): HttpHeaders {
  ...
  getStatus(id: number): Observable<Status> {
    const headers = this.getAuthHeaders();
    return this.http.get<Status>(`${this.apiUrl}/${id}`, { headers }).pipe(
  ...
  createStatus(status: Status): Observable<Status> {
    const headers = this.getAuthHeaders();
    return this.http.post<Status>(this.apiUrl, status, { headers }).pipe(
      catchError(this.handleError)
    );
  }
  ...
  updateStatus(id: number, status: Status): Observable<Status> {
    const headers = this.getAuthHeaders();
    return this.http.put<Status>(`${this.apiUrl}/${id}`, status, { headers }).pipe(
      catchError(this.handleError)
    );
  }
